...
func (l *Lot) CalcBoundingRect() {
	if len(((*l).contourPoints)) != 0 {
		...
		(*l).boudingRect = gocv.BoundingRect(gocv.NewPointVectorFromPoints((*l).contourPoints[0]))
		...
		gocv.DrawContours(&(*l).mask, gocv.NewPointsVectorFromPoints(contours), -1, color.RGBA{255, 255, 255, 255}, -1)
		//gocv.NewPointsVectorFromPoints(initialParameters.ParkingLots[i].GetContourPoints())
	} //else {

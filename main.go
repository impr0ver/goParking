...
func Grabber() {
	...
	for boolProc.Load() == true {
		...
		outBuf, _ := gocv.IMEncode(".bmp", singleFrame.FrameMat)
		//if err != nil {
		// Do not handle error
		//}

		singleFrame.Buf = outBuf.GetBytes()
...
func ProcessingData(f *framedata.FrameData) {
	...
	for i := range initialParameters.ParkingLots {
		...
		if meanMask > initialParameters.PJSON.Laplacian {
			...
			gocv.DrawContours(&imgCopy.FrameMat, gocv.NewPointsVectorFromPoints(initialParameters.ParkingLots[i].GetContourPoints()), -1, color.RGBA{255, 0, 0, 0}, 1)

			//Some work with img capture!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			for number <= 3 {
				timeStr := time.Now().Format("2006-01-02_15_04_05")
				pathName := fmt.Sprintf("results/output/parkingImage_%s_%s.jpg", initialParameters.ParkingLots[i].GetID(), timeStr)
				gocv.IMWrite(pathName, imgCopy.FrameMat)
				number++
			}

		} else {
			gocv.DrawContours(&imgCopy.FrameMat, gocv.NewPointsVectorFromPoints(initialParameters.ParkingLots[i].GetContourPoints()), -1, color.RGBA{0, 255, 0, 0}, 1)
		}
...
func main() {
	...
	//wg.Add(1)		  //
	//go Processing() //4 linux and windows
	Processing() //4 mac (windows.WaitKey() not working in goroutine, crash program)
